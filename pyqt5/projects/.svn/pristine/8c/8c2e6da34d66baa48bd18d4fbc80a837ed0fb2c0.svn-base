...
class uiMain( fc_hmi.Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    server_monitor_Signal = QtCore.pyqtSignal(str)

    def __init__(self):
        super(uiMain,self).__init__()

         # DCDC, CRM, and MRS Config parameters
        try:
            self.dcdc_config = configparser.ConfigParser()
            f = open(CONFIG_DIR + r'\dcdc_configuration.ini')
            self.dcdc_config.read_file(f)
            f.close()
            self.crm_config = configparser.ConfigParser()
            f = open(CONFIG_DIR + r'\crm_configuration.ini')
            self.crm_config.read_file(f)
            f.close()
            self.mrs_config = configparser.ConfigParser()
            f = open(CONFIG_DIR + r'\mrs_configuration.ini')
            self.mrs_config.read_file(f)
            f.close()
        ...
        self.cmd_win = cmd_hmi.Ui_CmdWindow()
        self.w_cmd = QtWidgets.QMainWindow()
        self.cmd_win.setupUi(self.w_cmd)
        qs = self.w_cmd.size()
        self.w_cmd.setFixedSize(qs)
        ...
        # # accessing each entry in ini file;
        # for i in self.dcdc_config.sections():
        #     print(i)
        #     for a in self.dcdc_config[i]:
        #         print(a)
        #         print(self.dcdc_config[i][a])
    ...
    def remap(self):
        ...
        for x in range(1,5):
            ...
            self.battery_tab.append(self.findChild(QtWidgets.QTabWidget, "tabWidget_crm_batteries_" + str(x)))
            ...
            for i in range(1,9):
                ...
                battery["cfet_" + str(i)] = self.findChild(QtWidgets.QLineEdit, "lineEdit_bat_cfet_" + str(i + bat_mapper))
                battery["dfet_" + str(i)] = self.findChild(QtWidgets.QLineEdit, "lineEdit_bat_dfet_" + str(i + bat_mapper))
    ...
    def __connect_signal_slots(self):
        ...
        self.server_monitor_Signal.connect(self.gui_monitor_slot)
        ...
        self.pushButton_cmd.clicked.connect(self.button_open_cmd)
        self.cmd_win.pushButton_cmd.clicked.connect(self.send_cmd)
        self.cmd_win.lineEdit_cmd.returnPressed.connect(self.cmd_win.pushButton_cmd.click)
    ...
    def send_cmd(self,flag):
        data = self.cmd_win.lineEdit_cmd.text()
        print("CMD: " + data)
        jsonData = {}
        jsonData["command"] = str(data)
        self.serverIx.send(str.encode(json.dumps(jsonData)))   

    def button_open_cmd(self,flag):
        self.toggle_cmd_window(self.w_cmd)

    def button_reset_crm1(self,flag):
        print("Reset CRM1")
        if True: #self.pushButton_reset_crm_1.text() == "Reset Needed":
            self.send_crm_reset(1) 
            self.pushButton_reset_crm_1.setText("Resetting")

    def button_reset_crm2(self,flag):
        print("Reset CRM2")
        if self.pushButton_reset_crm_2.text() == "Reset Needed":
            self.send_crm_reset(2)  
            self.pushButton_reset_crm_2.setText("Resetting")

    def button_reset_crm3(self,flag):
        print("Reset CRM3")
        if self.pushButton_reset_crm_3.text() == "Reset Needed":
            self.send_crm_reset(3) 
            self.pushButton_reset_crm_3.setText("Resetting")

    def button_reset_crm4(self,flag):
        print("Reset CRM4")
        if self.pushButton_reset_crm_4.text() == "Reset Needed":
            self.send_crm_reset(4) 
            self.pushButton_reset_crm_4.setText("Resetting") 
    ...
    def handle_dcdc_json(self, dcdc_id, dic):
         try:
            ...
            if self.tabWidget_DCDC.currentIndex() == id:
    ...
    def handle_crm_json(self,crm_id,dic):
        try:
            ...
            if self.tabWidget_CRM.currentIndex() != id:
                return
            ...
            for i in range(1,9):
                ...
                if((bat_json["discharge_fet_status"] == 0) | 
                   (bat_json["comm_status"] == 0) | 
                   (bat_json["charge_fet_status"] == 0)):
                    self.battery_tab[id].tabBar().setTabTextColor(i-1,QtCore.Qt.red) 
                else:
                    self.battery_tab[id].tabBar().setTabTextColor(i-1,QtCore.Qt.black)  

                if self.battery_tab[id].currentIndex() == i-1:  
                    ...
                    battery["cfet_" + str(i)].setText(str("Fault" if bat_json["charge_fet_status"] == 0 else "OK"))
                    battery["cfet_" + str(i)].setStyleSheet(
                        "background-color:" + ("orange" if bat_json["charge_fet_status"] == 0 else "lightgreen"))
                    
                    battery["dfet_" + str(i)].setText(str("Fault" if bat_json["discharge_fet_status"] == 0 else "OK"))
                    battery["dfet_" + str(i)].setStyleSheet(
                        "background-color:" + ("orange" if bat_json["discharge_fet_status"] == 0 else "lightgreen"))
    ...
    def handle_mrs_json(self,mrs_id,dic):
        try:
            ...
            if self.tabWidget_MRS.currentIndex() == id:
                for i in range(1,14):
                    self.mrs_analogs[id][i-1].setText(str(dic["AnalogChannel"+str(i)] / 1000))
    ...
    def handle_system_json(self,dic):
        ...
        if( str(dic["faults"]) == "NO FAULT"):
            self.textEdit_system_status.setStyleSheet("background-color: lightgray")
        else:
            self.textEdit_system_status.setStyleSheet("background-color: red")
    ...
    def gui_monitor_slot(self,status):
        if(self.imc == self.pimc):
            # server not connected
            self.label_server_status.setText("Server Offline!")
            self.textEdit_system_status.setText("Server is not communicating!")
            self.textEdit_system_status.setStyleSheet("background-color: red")
        else:
            self.pimc = self.imc
    ...
    def __monitor_thread_processor(self):
        val = 0.0
        while True:
            self.server_monitor_Signal.emit("None")
            time.sleep(2)
    ...
    def toggle_cmd_window(self, window):
        if window.isVisible():
            window.hide()
